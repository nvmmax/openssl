...
=item SSL_OP_DISABLE_KTLS_TX_ZEROCOPY_SENDFILE

By default, sendfile() will use the zerocopy mode on supported Linux kernels,
which gives a performance boost when used with kTLS device offload. Note that
invalid TLS records might be transmitted if the file is modified while being
sent, so this option can be used to disable zerocopy. This option has no effect
if B<SSL_OP_ENABLE_KTLS> is not enabled.
