...
B<KTLSTxNoZerocopySendfile>: disable the zerocopy TX mode of sendfile() on
Linux. This mode is enabled by default on supported kernels and gives a
...
option can be used to disable zerocopy. This option has no effect if B<KTLS> is
not enabled. Equivalient to B<SSL_OP_DISABLE_KTLS_TX_ZEROCOPY_SENDFILE>.
